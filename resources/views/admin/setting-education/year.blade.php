                ...
                "render": function (data, type, row, meta) {
                    ...
                    actions += `<div class="col-auto">
                                    <a class="btn btn-sm btn-success" type="button" href="{{url("/admin/setting-education/subject")}}/${data.id}">
                                        <i class="fa fa-home"></i> Subject
                                    </a>
                                </div>`;
                    actions += `</div>`;

                    if(meta.row === 0){
                        $("[data-bread=country]").text("({{ __("attributes.country") }}) " + data.stage.country.country.translate).attr("href", APP_URL + "/" + "admin/setting-education/stage");
                        $("[data-bread=stage]").text("({{ __("attributes.stage") }}) " + data.stage.stage.translate).attr("href", APP_URL + "/" + "admin/setting-education/stage");
                    }
                    return actions;
                }
            }
