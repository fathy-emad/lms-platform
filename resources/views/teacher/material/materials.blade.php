...
@section('breadcrumb-items')
    ...
    <li class="breadcrumb-item active">{{ __("lang.materials") }}</li>
@endsection
@section('content')
...
{{--                                            </div>--}}
        ...
        <div class="modal fade update_modal" aria-labelledby="myLargeModalLabel" style="display: none;" data-bs-backdrop="static" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title" id="myLargeModalLabel">{{ __('lang.update') }} Materials</h4>
                        <button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Close" data-bs-original-title="" title=""></button>
                    </div>
                    <div class="modal-body">
        ...
        <div class="modal fade view_modal" aria-labelledby="myLargeModalLabel" style="display: none;" data-bs-backdrop="static" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title" id="myLargeModalLabel">{{ __('lang.view') }} Materials</h4>
                        <button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Close" data-bs-original-title="" title=""></button>
                    </div>
                    <div class="modal-body">
...
@section('script')
    ...
    <script>
        ...
        let datatableUri = `{{ url("api")."/teacher/material?where=course_id:" . request("course_id")}}`;
                ...
                "render": function (data, type, row, meta) {
                    ...
                    actions += `<div class="col-auto">
                                <button class="btn btn-sm btn-warning" type="button" onclick="openModalUpdate(${dataString})"
                                        data-bs-original-title="{{ __('lang.update') }} Material"
                                        title="{{ __('lang.update') }} Material">
                    ...
                    actions += `<div class="col-auto">
                                <button class="btn btn-sm btn-primary" type="button" onclick="openModalView(${dataString})"
                                        data-bs-original-title="{{ __('lang.view') }} Material"
                                        title="{{ __('lang.view') }} Material">
                                    <i class="fa fa-eye"></i>
                                </button>
                            </div>`;
                    actions += `</div>`;
        ...
        function openModalUpdate(data) {
            ...
            modal.find("[name=lesson_id]").val(data.lesson_id).trigger("change");
        ...
        function openModalView(data) {
            ...
            questionsUpdate = [];
            questionsUpdate = data.assignment;
            modal.find("[name=lesson_id]").val(data.lesson_id).trigger("change").prop("disabled", true);
        ...
        $(document).ready(function() {
            //Get Lessons
            $.ajax({
                url: APP_URL + "/api/teacher/lesson?where=chapter_id:{{ request("chapter_id") }}",
                type: "GET",
                data: null,
                processData: false,
                contentType: false,
                ...
                success: function(response) {
                    ...
                    $("[data-bread=chapter]").text("({{ __("attributes.chapter") }}) " + data[0].chapter.chapter.translate)
                        .attr("href", APP_URL + "/" + "teacher/material/{{request("course_id")}}/{{request("curriculum_id")}}");
                    ...
                    $(".create_modal, .update_modal, .view_modal").each(function() {
                        ...
                        $(this).find("#lesson_id").on("change", function () {
                            ...
                            //Get bank-questions
                            $.ajax({
                                url: APP_URL + "/api/teacher/bank-question?where=teacher_id:{{ session("teacher_data")["id"] }},lesson_id:"+lesson_id+",ActiveEnum:{{\App\Enums\ActiveEnum::Active->value}}"+
            ...
            //Get languages
            $.ajax({
                url: APP_URL + "/api/teacher/language",
                type: "GET",
                data: null,
                processData: false,
                contentType: false,
