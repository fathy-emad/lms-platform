...
@section('script')
    ...
    <script>
        let datatableUri = `{{ url("api")."/teacher/lesson?where=chapter_id:" . request('chapter_id') .",ActiveEnum:".\App\Enums\ActiveEnum::Active->value}}`;
            ...
            { "data": "lesson.translate" },
            {
                ...
                "render": function (data, type, row, meta) {
                    ...
                    actions += `<div class="col-auto">
                                    <a class="btn btn-sm btn-success" type="button" href="{{url("/teacher/bank-question/".request("curriculum_id")."/".request("chapter_id"))}}/${data.id}">
                                        <i class="fa fa-home"></i> Questions
                                    </a>
                                </div>`;
                    actions += `</div>`;
                    if(meta.row === 0){
                        $("[data-bread=curriculum]").text("({{ __("attributes.curriculum") }}) " + data.chapter.curriculum.curriculum.translate).attr("href", APP_URL + "/teacher/bank-question");
                        $("[data-bread=chapter]").text("({{ __("attributes.chapter") }}) " + data.chapter.chapter.translate).attr("href", APP_URL + "/teacher/bank-question/{{request("curriculum_id")}}");
                    }
                    return actions;
                }
            }
